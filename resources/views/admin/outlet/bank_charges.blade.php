            ...
            <div class="card-body table-responsive py-4">
                <table id="table" class="table table-hover table-sm text-nowrap">
                    ...
                    <tbody>
                        ...
                        @php
                        $from_state =  implode(",", $bank['from_state']);
                        $to_state =  implode(",", $bank['to_state']);
                        @endphp
                        <tr>
                            <td>{{ ++$i }}</td>
                            <td>{!! !empty($bank['api_id']) ? getAPIName($bank['api_id']) : '' !!}</td>
                            <td>{!! !empty($bank['from_state']) ? $from_state : mSign(0) !!}</td>
                            <!-- <td>{!! !empty($bank['from_city']) ? ($bank['from_city']) : mSign(0) !!}</td> -->
                            <td>{!! !empty($bank['to_state']) ? $to_state : mSign(0) !!}</td>
    ...
    $(document).on('click', '.edit_bank_account', function(e) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.data.weight_range)
                    res.data.weight_range.forEach(function(e,key) {
                      if(key == 0){
                      $('#min_weight').val(e.min_weight);
                      $('#max_weight').val(e.max_weight);
                      $('#charges').val(e.charges);
                      }else{
                             ...
                             <div class="addcharges">\
                                   <div class="form-row">\
                                    ...
                                    <div class="form-row">\
                                        <div class="form-group col-md-6">\
                                            <label>Charges</label>\
                                            <input type="number" step="any" name="charges[]" value="' + e.charges + '"  id="charges" class="form-control form-control-sm" placeholder="Enter Charges">\
